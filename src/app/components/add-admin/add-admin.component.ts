...
export class AddAdminComponent implements OnInit {
  ...
  SubmitForm() {
    let ProfileImageURL = (<HTMLInputElement>this.AdminImage.nativeElement).files![0];
    this.AdminAuth.SignUp(this.Email?.value, this.Password?.value)
      .then((data) => {
    this.AdminServ.AddAdmin(data.user?.uid,this.Email?.value,this.Name?.value , ProfileImageURL)
    .then(() => {
